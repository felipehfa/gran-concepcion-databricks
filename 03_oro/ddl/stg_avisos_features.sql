-- Tabla: gran_concepcion.03_oro.stg_avisos_features
-- Creada por: 03_oro/06_features_oro_sql.py (sección 14, `CREATE TABLE ... AS
-- SELECT` a partir de `pendientes_oro`) — NO tiene un DDL explícito en el
-- notebook, el esquema lo infiere Spark de la cadena de vistas temporales
-- (misma situación que avisos_limpios en Plata). Columnas agregadas después
-- vía `ALTER TABLE ADD COLUMNS`: `uv_rsh`/`fecha_vulnerabilidad_oro`
-- (07_vulnerabilidad_oro_python.py) y
-- `fecha_chequeo_estado_oro`/`intentos_fallidos_chequeo_estado_oro`
-- (09_actualizacion_estado_avisos_oro_python.py).
--
-- Este archivo se generó leyendo el esquema real de la tabla en el catálogo
-- (`DESCRIBE TABLE`), no transcribiendo el SELECT a mano. Es referencia de
-- solo lectura: correrlo no reemplaza el `CREATE TABLE ... AS SELECT` real.
--
-- OBT — el feature store completo: hereda ~90 columnas de avisos_limpios
-- (texto crudo + parseado) y agrega las ~15 features calculadas por Oro.
-- Fuente de las 4 dimensiones satélite (dim_descripcion_propiedad,
-- dim_amenidades, dim_ubicacion vía dim_barrio, y dim_estado_aviso_scd2) que
-- pobla 11_modelo_dimensional_oro_sql.py.
--
-- Notas sobre el estado real observado al momento de escribir este archivo:
--   * `fecha_chequeo_estado_oro`/`intentos_fallidos_chequeo_estado_oro` están
--     declaradas abajo como parte del esquema completo del diseño, pero
--     `09_actualizacion_estado_avisos_oro_python.py` todavía no había
--     corrido contra este catálogo — esas dos columnas pueden no existir
--     todavía en la tabla real hasta la primera corrida de ese notebook.
--   * `rn` (BIGINT/INT) aparece al final de la tabla real, sobrante del
--     dedup de la sección 15 (`ROW_NUMBER() ... AS rn`, `WHERE rn = 1`,
--     `CREATE OR REPLACE TABLE ... AS SELECT`). Es un bug latente del
--     notebook original: `seleccion_base` solo excluye `rn` de las columnas
--     ORIGINALES, pero el `SELECT` siempre vuelve a agregar `rn` al final —
--     nunca se limpia. No se declara en el DDL de abajo (no debería existir
--     como columna permanente); si aparece en `DESCRIBE TABLE`, es la señal
--     de que ese bug sigue sin corregirse en 06_features_oro_sql.py.
--   * `solo_familias_texto` puede reportar tipo `void` en `DESCRIBE TABLE`
--     si el 100% de las filas actuales tienen NULL ahí — se declara STRING
--     abajo, el tipo que le corresponde.

CREATE TABLE gran_concepcion.03_oro.stg_avisos_features (
    id_aviso                                    STRING,
    comuna                                       STRING,
